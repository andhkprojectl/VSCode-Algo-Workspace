...
2. **BB cross value** = save the **close price** at the cross bar (NaN otherwise). Diff1/3/5 = future close[t+1/+3/+5] - close[cross].
3. **EMA typo** = use cDiffEma5/10/20; percentile vars named `cDiffEma20_90` / `cDiffEma20_10` (treat "15" in requirement as typo for "20").
...
7. **EMA group**: ema5/10/20 = close.ewm(span=N, adjust=False).mean(); cDiffEma{N} = close - ema{N}. Percentiles cDiffEma{N}_90/_10 (N=5,10,20).
8. **BB group**: BB period=20, std=2 (reuse `test_dia_ym_correlation1.py` pattern). top=mean+2std, bot=mean-2std. Cross-up: prev close<=top & cur close>top → cCrossUpBBTop=close. Cross-down: prev close>=bot & cur close<bot → cCrossDownBBBottom=close. Diff1/3/5 = close.shift(-1/-3/-5) - cross_close. Percentiles for all 8 BB cols.
...
14. Build feature list (all 112 stat cols). Build clean df = drop rows with any NaN in features or rt targets.
...
16. **Pairs**: closed-form 2-feature R^2 from correlations: `R2 = (r_y1^2 + r_y2^2 - 2*r_y1*r_y2*r_12)/(1-r_12^2)`. Iterate all C(112,2)=6216 pairs × 4 rt. Strong if R2>=0.25. (Validate closed-form vs sklearn LinearRegression on 5 random pairs in tests.)
...
18. `S6002_1_statistics_revenue.csv`: input cols + all stats + rt cols (per-row).
...
24. Tests (synthetic small dataframes with known values):
    ...
    - test_bb_cross_up/down: constructed cross → cCrossUpBBTop = close, Diff1/3/5 correct.
...
6. Spot-check: `atr3` at row 105 ≈ mean of last 3 TR values; `rsi14` row 20 in 0-100 range; `cDiffEma20_90` finite after row 100.

## Decisions
- ATR = True Range (H/L/C), not close-only.
- BB cross stores close price at cross bar; Diff = forward close minus cross close.
- EMA set = {5,10,20}; percentile "15" treated as typo for "20".
