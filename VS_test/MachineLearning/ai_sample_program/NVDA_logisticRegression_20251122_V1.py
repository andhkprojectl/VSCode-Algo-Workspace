...
def fillMissPrePstOpenCloseTIme(df1):
    ...
    target_times = ['04:01:00', '09:30:00', '15:58:00', '19:58:00']
    # print("110")
...
def calMatrix(df1, isReg):
    ...
    # sma20
    dfReg['sma1'] = dfReg['Close'].rolling(window=20).mean()
    ...
    # num of days (tomorrow 09:30 Open - today 16:00 or 20:00 close > 0 )
    # num of days (tomorrow 09:30 Open - today 16:00 or 20:00 close < 0 )
    ...
    for i1, row1 in dfReg.iterrows():
        ...
        perCDiffSma1 = row1['perCDiffSma1']

        # if per not exists, add to value
        # matrix1['perCDiffSma1'] = perCDiffSma1
        # print(f"1000. {perCDiffSma1}")
        if not (matrix1['perCDiffSma1'] == perCDiffSma1).any():
            # print(f"1001. {perCDiffSma1}")
            new_row = {
            #    'perCDiffSma1_index': perCDiffSma1,
                'perCDiffSma1': perCDiffSma1,
                'numDaysNextDayPos': 0,
                'numDaysNextDayNeg': 0,
                'numDaysCurrDayPos': 0,
                'numDaysCurrDayNeg': 0
            ...
            # new_index = perCDiffSma1
            # new_row_df = pd.DataFrame([new_row], index=[new_index])
            new_row_df = pd.DataFrame([new_row])
            matrix1 = pd.concat([matrix1, new_row_df])
            matrix1['perCDiffSma1_index'] = matrix1['perCDiffSma1']
            matrix1 = matrix1.set_index('perCDiffSma1_index', drop=True)
        ...
        if (currDayReturn > 0):
            matrix1.loc[perCDiffSma1, 'numDaysCurrDayPos'] =  currMatrixRow['numDaysCurrDayPos'] + 1
        elif (currDayReturn < 0):
            matrix1.loc[perCDiffSma1, 'numDaysCurrDayNeg'] = currMatrixRow['numDaysCurrDayNeg'] + 1
    ...
    df2 = dfReg2.merge(matrix1, on='perCDiffSma1', how='left')
    # df2 = dfReg.merge(matrix1_with_col, on='perCDiffSma1', how='left')
    df2 = df2.bfill()
    print(df2.columns.tolist())
    # → ['Date1', 'Time1', 'perCDiffSma1', ..., 'numDaysNextDayPos', ...]

    # df2.to_csv("c:/tmp/calMatrix2.csv")
...
def buildFeaturePd(df1, df_matrix):
    ...
    for date, group in df1.groupby('Date1'):
        ...
        for time, close, open, vol in zip(group['time'], group['Close'], group['Open'], group['Volume']):
            ...
            if time.hour == 9 and time.minute == 30:
                ...
                v_pre_0930[date] = vol
                print("201_5")
            elif time.hour == 15 and time.minute == 58:
                print("201_6")
                daily_1558_C[date] = close
                v_reg_1558[date] = vol
                print("201_7")
            elif time.hour == 19 and time.minute == 58:
                print("201_8")
    ...
    for i, row in df1.iterrows():
        ...
        # print(f"888_1")
        if current_time.hour == 19 and current_time.minute == 58 and tomorrow1 is not None:
            print(f"111_0")
            ...
            print(f"111_1")
        if current_time.hour == 15 and current_time.minute == 58 and tomorrow1 is not None:
        ...
        if current_time.hour == 4 and current_time.minute == 1:
            ...
            preAccVol = 0
            print(f"116_2")
        if current_time.hour == 19 and current_time.minute == 58:
            print(f"1199_0")
    ...
    df1Daily1958 = df1[df1['datetime'].dt.time == pd.to_datetime('19:58:00').time()]
...
def performLogisticRegressionNVDAPositive(df0):
    ...
    features = [ 'sma10diff20', 'rsi14', 'vPstOverReg', 'sma10', 'vReg', 'vPre', 'vPst', 'Volume'
                 ...
                 # , 'numDaysNextDayPos', 'numDaysNextDayNeg'
                 , 'numDaysCurrDayPos', 'numDaysCurrDayNeg', 'Open'
                 , 'rsi14_l_50', 'sma10_l_20', 'numPos_l_numNeg'
                ]
