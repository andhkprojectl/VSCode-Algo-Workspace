...
# 1. Target symbol: DIA
# 2. Target:
# 	- 30m return from 9:30 to 12:30
# 	- Fix return USD100, stop loss USD50.00
# 3. use linear regression and random forest to study what features highly correlated with DIA
# 4. 30min time frame each bar
# 5. intrday, perform from 9:30 to 12:30 (winter time 22:30 to 13:30)
# 6. Features (misc.xlsx)
# 	- prev DIA 30min return (at 9:30, prev 30min 9:00)
# 	- prev DIA 30min return (at 9:30, prev 30min 15:30)
# 	- prev DIA post trading period return (return from 16:00 to 20:00)
# 	- prev DIA pre trading period return (return from 4:00 to 09:30)
# 	- prev DIA regular trading period return (return from 16:00 to 09:30)
# 	- prev @YM#C 30min return
# 	- prev @YM#C 60min return
# 	- prev @YM#C 120min return
# 	- prev 30min DIA volume (at 9:30, prev 30min 9:00)
# 	- prev 30min DIA volume (at 9:30, prev 30min 15:30)
# 	- above percentile
#
# - Note:
...
def prepare_data(start_dt1, end_dt1):
    ...
    try:
        ...
        # dia_data1 = fetch_daily_data(conn, "DIA", start_dt1, end_dt1)
        dia_data1 = fetch_data_with_time(conn, "DIA", start_dt1, "04:00", end_dt1, "20:30", 1800)
        # dia_data1 = fetch_data_with_time(conn, "DIA", start_dt1, "09:30", end_dt1, "16:00", 1800)
...
def plot_interactive_chart(strategy, data_feed):
    # Extract data for plotting
    df = pd.DataFrame({
        'datetime': data_feed.datetime.array,
        'Open': data_feed.open.array,
        'High': data_feed.high.array,
        'Low': data_feed.low.array,
        'Close': data_feed.close.array,
        'Volume': data_feed.volume.array
    })
    df['datetime'] = pd.to_datetime([bt.num2date(dt) for dt in df['datetime']])

    # Extract buy and sell signals
    buy_signals = [order for order in strategy.order_history if order['type'] == 'Buy']
    sell_signals = [order for order in strategy.order_history if order['type'] in ['Normal Sell', 'Stop Loss', 'Take Profit']]
    ...
    # # Add buy signals
    # if buy_signals:
    #     buy_dates = [signal['datetime'] for signal in buy_signals]
    ...
    # # Add sell signals
    # if sell_signals:
    #     sell_dates = [signal['datetime'] for signal in sell_signals]
    ...
    # Create subplot with 2 rows (candlestick and volume)
    fig = make_subplots(
        rows=2, cols=1,
        row_heights=[0.7, 0.3],
        shared_xaxes=True,
        vertical_spacing=0.05,
        subplot_titles=('DIA Price with Buy/Sell Signals', 'Volume')
    )
    ...
    if buy_signals:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                # text=[f"Buy Signal: ${price:.2f}" for price in buy_prices],
                # Enhanced hover text
                # hoverinfo='text',
                name='Buy Signal'
    ...
    if sell_signals:
        sell_dates = [signal['datetime'] for signal in sell_signals]
        sell_prices = [signal['price'] for signal in sell_signals]
        # sell_types = [signal['type'] for signal in sell_signals]
        fig.add_trace(
            go.Scatter(
                x=sell_dates,
                y=sell_prices,
                ...
                # text=[f"{sell_type}: ${price:.2f}" for sell_type, price in zip(sell_types, sell_prices)],
                # Enhanced hover text
                # hoverinfo='text',
                name='Sell Signal'
            ),
    ...
    fig.update_layout (
        title='DIA Price with Buy/Sell Signals',
        xaxis_title='Date',
        yaxis_title='Price',
        xaxis2_title='Date',
        yaxis2_title='Volume',
        ...
        yaxis=dict(fixedrange=False), # Allow zoom in/out with mouse
        dragmode = 'pan',
        showlegend=False
        # xaxis=dict(
        #     range=[df['datatime'].values[-1] - timedelta(days=30), df['datatime'].values[-1]],  # Set default range to last 30 days
        #     rangeslider_visible=False
        # )
    )

    # Set default x-axis range last 30 days of last values
    chartXEndDt1 = df['datetime'].iloc[-1]
    chartXDays = 120
    fig.update_xaxes(range=[chartXEndDt1 - timedelta(days=chartXDays), chartXEndDt1], row=1, col=1)
    fig.update_xaxes(range=[chartXEndDt1 - timedelta(days=chartXDays), chartXEndDt1], row=2, col=1)
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                ...
                # Print trade details in the requested format
                # print("\nOrder Details:")
                # print(f"DIA,{buy_dt.strftime('%Y-%m-%d %H:%M:%S')},{buy_price:.2f},"
                #       f"{sell_dt.strftime('%Y-%m-%d %H:%M:%S')},{sell_price:.2f},{profit:.2f},{size}")
    ...
    def stop(self):
        ...
        if len(self.portfolio_values) > 1:
            ...
            if np.std(returns) > 0:
                ...
                # manual_sharpe = np.mean(excess_returns) / np.std(returns)
...
#    print("inDataList[0]. DIA")
#    print(inDataList[0].p.head())
    ...
    # below get trade list
    # Example: extract per-trade returns
    tradeList1 = results[0].analyzers.trades.get_analysis()
    # Loop over trades['closed'] list and compute return, days held
    ...
    # plt.show()
    if isPlot:
        plot_interactive_chart(backTestReturn, inDataList[0])
