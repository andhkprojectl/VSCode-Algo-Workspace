...
def merge_2_dataFrame(df1, df2):
    ...
    df4 = df4.rename(columns={'Date1': 'Date1_1', 'Time1': 'Time1_1', 'Open': 'Open_1', 'High': 'High_1' , 'Low': 'Low_1', 'Close': 'Close_1', 'Volume': 'Volume_1', 'symbol': 'symbol_1'})
...
def fillMissingData(df1):
    ...
    df1[['Date1_1', 'Time1_1', 'Open_1', 'High_1', 'Low_1', 'Close_1', 'Volume_1', 'symbol_1', 'Date1_2', 'Time1_2', 'Open_2', 'High_2', 'Low_2', 'Close_2', 'Volume_2', 'symbol_2']] = df1[['Date1_1', 'Time1_1', 'Open_1', 'High_1', 'Low_1', 'Close_1', 'Volume_1', 'symbol_1', 'Date1_2', 'Time1_2', 'Open_2', 'High_2', 'Low_2', 'Close_2', 'Volume_2', 'symbol_2']].ffill()
...
def featureScaling2(df1):
    ...
    columns_to_scale = [
        ...
        , 'rtPre_2', 'rtReg_2', 'rtPst_2', 'rtPrePst_2', 'combine_2_1_p', 'combine_2_2_p', 'combine_2_3_p', 'combine_2_4_p'
        , 'rtPre_1', 'rtReg_1', 'rtPst_1', 'rtPrePst_1'
        , 'ptl_top_reach_2_p', 'ptl_bottom_reach_2_p', 'vol2_top_reach_2_p'
        , 'ema25_2_p', 'ema50_2_p', 'ema_diff_2_p', 'ema_cross_2_p', 'per_rt_2_p', 'volatility_2_p', 'rsi_2_p'
    ]
...
def buildFeaturePd(df1):
    ...
    # ym_dia_data.to_csv("c:/tmp/ym_dia_test2.csv")

    # add features and target to ym_dia_data
    ...
    topPercentileValue2 = 0.8
    bottomPercentileValue2 = 0.2
    percentilePeriod = 100  # days

    # find return percentile 10% and 90%
    df1['ptl_top_1'] = df1['rt_1'].rolling(window=percentilePeriod).quantile(
        topPercentileValue)
    df1['ptl_bottom_1'] = df1['rt_1'].rolling(window=percentilePeriod).quantile(
        bottomPercentileValue)
    ...
    # Compute indicators
    df1['ema25_2'] = df1['Close_2'].ewm(span=25).mean()
    df1['ema50_2'] = df1['Close_2'].ewm(span=50).mean()
    df1['ema_diff_2'] = df1['ema25_2'] - df1['ema50_2']
    df1['ema_cross_2'] = (df1['ema25_2'] > df1['ema50_2']).astype(int)

    # Momentum and volatility features
    df1['per_rt_2'] = df1['Close_2'].pct_change()
    df1['volatility_2'] = df1['per_rt_2'].rolling(20).std()
    df1['rsi_2'] = df1['per_rt_2'].rolling(14).apply(
        lambda x: 100 - 100 / (1 + (x[x > 0].mean() / (-x[x < 0].mean() if -x[x < 0].mean() != 0 else 1))), raw=False)
    df1['hour'] = df1['datetime'].dt.hour
    df1['dayofweek'] = df1['datetime'].dt.dayofweek
    df1['dateofmth'] = df1['datetime'].dt.day


    df1['ema25_2_p'] = df1['ema25_2'].shift(1)
    df1['ema50_2_p'] = df1['ema50_2'].shift(1)
    df1['ema_diff_2_p'] = df1['ema_diff_2'].shift(1)
    df1['ema_cross_2_p'] = df1['ema_cross_2'].shift(1)
    df1['per_rt_2_p'] = df1['per_rt_2'].shift(1)
    df1['volatility_2_p'] = df1['volatility_2'].shift(1)
    df1['rsi_2_p'] = df1['rsi_2'].shift(1)
    ...
    # remove last
    # ym_data = ym_data.iloc[:-1]

    # remove 1st value. remove diff
    # ym_data = ym_data.iloc[1:]

    # remove last
    # dia_data = dia_data.iloc[:-1]
    # remove 1st value. remove diff
    # dia_data = dia_data.iloc[1:]
    ...
    # feature1['Target'] = dia_data['nextDay_return1']

    # print("feature:", feature1.shape)
    # print("dia_data:", dia_data.shape)
    # print("ym_data:", ym_data.shape)

    # Features and target arrays
    # features = ['Open', 'High', 'Low', 'Close', 'Volume', 'MA5', 'MA10', 'Return1', 'VolumeChange']
    # X = feature1
    # y = dia_data['nextDay_return1']
...
def performCorrelation(df1):
    ...
    # Save to new CSV file
    df2.to_csv('c:/tmp/performCorrelation.csv', index=False)
    ...
    # print("rt_2")
    print (df2_rt2)
    df2_rt2.to_frame(name="Correlation").to_csv("c:/tmp/performCorrelation2.csv", index_label="Field")
    ...
    for start, end in time_windows:
        ...
        if not subset.empty:
            ...
            for col in other_cols:
                plt.scatter(subset['rt_2'], subset[col], alpha=0.6, label=col)
...
def performLogisticRegressionDIA(df1):
    ...
    X = df2[['rt_1_p', 'rt_2_p', 'rsi_14_p', 'ptl_top_1_p', 'ptl_bottom_1_p', 'ema20_p', 'top_bb_p', 'bottom_bb_p', 'vol_2_diff_p']]
    y = df2['y']
...
def performLogisticRegressionYM(df1):
    # Compute features (X)
    # 1) Last 30min return = rt_1
...
# Target YM close price
def performLogisticRegressionYM2(df0):
    df1 = df0
    ...
    df2 = df1_2.dropna(subset=['rt_2_p_rank', 'vol_2_diff_p_rank'])

    df2['combined_rank'] = (df2['rt_2_p_rank'] * df2['vol_2_diff_p_rank']) / 10000
    df2['combined_rank1'] = (df2['rt_2_p_rank'] * df2['vol_2_diff_p_rank']) / df2['rt_2_p_rank'].max()*df2['vol_2_diff_p_rank'].max()
    df2['combined_rank2'] = (df2['rt_2_p_rank'] + df2['vol_2_diff_p_rank']) / (df2['rt_2_p_rank'].max()+df2['vol_2_diff_p_rank'].max())
    df2['combined_rank3'] = (df2['rt_2_p_rank'] * df2['vol_2_diff_p_rank']) / 10000
    df2['combined_rank4'] = (df2['rt_2_p_rank'] * df2['vol_2_diff_p_rank'])

    df2.to_csv("c:/tmp/performLogisticRegressionYM2.csv")
    ...
    features = [ # 'ptl_top_2_p', 'vol_2_diff_top_p', 'rtPre_2'
                ...
                'ptl_top_2_p', 'vol_2_diff_top_p',  'ptl_top_reach_2_p', 'ptl_bottom_reach_2_p', 'vol2_top_reach_2_p', 'ptl_bottom_2_p'
                # 'ptl_top_2_p'
                # 'vol_2_diff_top_p'
    ...
    for C in [0.01, 0.1, 1.0, 3.0, 10.0]:
        ...
        for train_idx, test_idx in tscv.split(X):
            ...
            print(f"C: {C}")
            print(f"Model Accuracy: {accuracy_score(y_te, y_pred):.4f}")
            print(f"Model Coefficients: {model.coef_}")
            #
            # # Model strategy success rate (when predicts buy, % positive outcomes)
            # model_success = y_test[y_pred == 1].mean() if np.any(y_pred == 1) else 0
            # print(f"Model Success Rate on Test: {model_success:.4f}")
    ...
    # print("AUC with auc_rtPre_2 only:", auc_rtPre_2)
    # print("AUC with ptl_top_reach_2_p only:", auc_4)
    # print("AUC with vol2_top_reach_2_p only:", auc_5)
...
def performRandomForestDIA(df1):
    ...
    X = df2[['rt_1_p', 'rt_2_p', 'rsi_14_p', 'ptl_top_1_p', 'ptl_bottom_1_p', 'ema20_p', 'top_bb_p', 'bottom_bb_p', 'vol_1_diff_p']]
    y = df2['y']
...
def performRandomForestYM(df1):
    ...
    # X = df2[['rt_1_p', 'rt_2_p', 'rsi_14_p', 'ptl_top_2_p', 'ptl_bottom_2_p', 'ema20_p', 'top_bb_p', 'bottom_bb_p', 'vol_2_diff_p'
            # , 'ptl_top_reach_2_p', 'ptl_bottom_reach_2_p', 'vol2_top_reach_2_p'
             # ]]
    # X = df2[[
    #        'ptl_top_2_p', 'vol_2_diff_p' , 'ptl_top_reach_2_p', 'ema25_2_p', 'ema50_2_p'
    #    , 'ema_diff_2_p', 'ema_cross_2_p', 'per_rt_2_p', 'volatility_2_p', 'rsi_2_p']]

    X = df2[[
         # 'ptl_bottom_2_p', 'bottom_bb_p', 'vol_2_diff_p', 'ema50_2_p', 'rsi_14_p'
        'ptl_bottom_2_p', 'bottom_bb_p', 'vol_2_diff_p', 'ema50_2_p', 'rsi_14_p'
            #'top_bb_p', 'bottom_bb_p', 'vol_2_diff_p'
        ]]
    y = df2['y']
    ...
    # print("Index min/max:", X.index.min(), X.index.max())
    # split_date = '2025-08-01'
    # print("Parsed split_date:", pd.to_datetime(split_date))
    # print("Rows before split:", (X.index <= pd.to_datetime(split_date)).sum())
    # print("Rows after split:", (X.index > pd.to_datetime(split_date)).sum())
    ...
    grid_search = GridSearchCV(
        rf_clf, param_grid, cv=tscv, scoring='accuracy', n_jobs=-1
        , error_score = 'raise'
    )
    ...
    df1.loc[X_test.index, 'prob_up'] = best_model.predict_proba(X_test)[:, 1]
    # datetime_index

    # Evaluate model
    auc = roc_auc_score(y_test, df1.loc[X_test.index, 'prob_up'])
    acc = accuracy_score(y_test, (df1.loc[X_test.index, 'prob_up'] > 0.5).astype(int))
...
def main():
    ...
    #
    # 6 months
    # start_dt1 = datetime(2025, 3, 1)
    # start_dt1 = datetime(2021, 1, 1)
    start_dt1 = datetime(2021, 10, 1)
    #
    # 1 year
    ...
    # end_dt1 = datetime(2025, 9, 30)
    # end_dt1 = datetime(2022, 12, 31)
    end_dt1 = datetime(2021, 10, 31)
    ...
    try:
        ...
        # dia_data = fetch_data_with_time(conn, symbol01, start_dt1, "03:00", end_dt1, "20:30", period1)
        dia_data = fetch_data_with_time(conn, symbol01, start_dt1, "00:00", end_dt1, "23:30", period1)
        # ym is CME future from 00:00:00 to 23:59:59
        ym_data = fetch_data_with_time(conn, symbol02, start_dt1, "00:00", end_dt1, "23:30", period1)
        ...
        # dia_data['symbol'] = symbol01
        # ym_data['symbol'] = symbol02

        dia_ym_combine_data1 = merge_2_dataFrame(dia_data, ym_data)

        #
        # Data cleaning
        #
        # remove holiday
        dia_ym_combine_data2 = rmHoliday(dia_ym_combine_data1)

        # dia_ym_combine_data1.to_csv("c:/tmp/dia_ym_combine_data1.csv")
        # dia_ym_combine_data2.to_csv("c:/tmp/dia_ym_combine_data2.csv")

        # Fill blank (NaN) values in these columns with the previous row's values (forward fill)
        dia_ym_combine_data3 = fillMissingData(dia_ym_combine_data2)

        # dia_ym_combine_data3 include pre-trading, regular and post-trading hour
        # (optional) below filter dia_ym_combine_data3, dia_ym_combine_data3 include all trading hour data
        # dia_ym_combine_data3_pre, dia_ym_combine_data3_reg, dia_ym_combine_data3_pst = filterPrePstTradingHour(dia_ym_combine_data3)
        # dia_ym_combine_data3 = dia_ym_combine_data3_reg
        # dia_ym_combine_data3 = dia_ym_combine_data3_pst

        #
        # Feature scaling
        #
        # only set @YM#C value /100, TBD
        # dia_ym_combine_data4 = dia_ym_combine_data3
        dia_ym_combine_data4 = featureScaling(dia_ym_combine_data3)
        ...
        # start_time3 = pd.to_datetime('09:30:00').time()
        # end_time3 = pd.to_datetime('16:30:00').time()
        # dia_ym_combine_data4 = dia_ym_combine_data4[(dia_ym_combine_data4['datetime'].dt.time >= start_time3) & (dia_ym_combine_data4['datetime'].dt.time <= end_time3)]


        #
        # build target features dataframe
        #
        dia_ym_combine_data5 = buildFeaturePd(dia_ym_combine_data4)

        # dia_ym_combine_data5_scaled = dia_ym_combine_data5
        dia_ym_combine_data5_scaled = featureScaling2(dia_ym_combine_data5)

        # filter1, try diff filter
        # dia_ym_combine_data6 = dia_ym_combine_data5
        # dia_ym_combine_data6 = filter1(dia_ym_combine_data5)
        ...
        # scatterMatrixChart(dia_ym_combine_data5)
        # scatterMatrixChart(dia_ym_combine_data6)
        ...
        # scatterPlot(dia_ym_combine_data5)
        # scatterPlot(dia_ym_combine_data5_scaled)
        ...
        scatterPlot3(dia_ym_combine_data5)
        # scatterPlot3(dia_ym_combine_data5_scaled)
        ...
        #
        # filter2. intraday
        # test_corr1_df = filter2(dia_ym_combine_data5)
        # performCorrelation(test_corr1_df)
        # performCorrelation(dia_ym_combine_data5_scaled)
        # performCorrelation(dia_ym_combine_data5)
        #
        #
        # 4. end correlation
        #
        ...
        #
        # use not scaled
        # performLogisticRegressionDIA (dia_ym_combine_data5)
        # performLogisticRegressionYM (dia_ym_combine_data5)
        # performLogisticRegressionYM2 (dia_ym_combine_data5)
        # performLogisticRegressionYM2 (dia_ym_combine_data5_scaled)

        # (not using this)
        # (not using this) performLogisticRegressionDIA(dia_ym_combine_data5_scaled)

        #
        # random forest
        #
        # below use RandomForestClassifier
        # performRandomForestDIA(dia_ym_combine_data5)
        # performRandomForestYM(dia_ym_combine_data5)
