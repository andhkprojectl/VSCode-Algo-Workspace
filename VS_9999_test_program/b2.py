...
df['Buy Date Time'] = pd.to_datetime(df['Buy Date Time'], unit='D', origin='1899-12-30')
df['Sell Date Time'] = pd.to_datetime(df['Sell Date Time'], unit='D', origin='1899-12-30')
...
num_trades = len(df)
winning_trades = len(df[df['Profit'] >= 0])  # Profit >= 0 for winning trades
losing_trades = len(df[df['Profit'] < 0])    # Profit < 0 for losing trades
